...
ret, thg = cv2.threshold(img_sobel, 127, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)

# kernel = np.ones((3, 3), np.uint8)
# close = cv2.morphologyEx(thg, cv2.MORPH_CLOSE, kernel)
#
# canny = cv2.Canny(img_sobel, 100, 100)

contours, hierarchy = cv2.findContours(thg, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
# print(len(contours))
cnts = sorted(contours, key=cv2.contourArea, reverse=True)[:40]#było 20
